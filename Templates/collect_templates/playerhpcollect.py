# playerhpcollect.py
# 模板采集工具 - 玩家生命值数字采集器
# 功能：通过GUI界面采集玩家生命值区域的数字模板，用于生命值识别
...
try:
    ...
    print("警告: 无法导入 get_window_rect，使用默认值替代")
    def get_window_rect():
...
# ===== 截图区域定义 =====
# 定义玩家生命值显示区域的截图位置（请根据你的屏幕实际位置调整）
# 格式：(x坐标, y坐标, 宽度, 高度)
REGION = (948 + x_o, 675 + y_o, 60, 40)  # 示例：玩家生命值显示区域
...
# 创建主窗口
root = tk.Tk()
root.title("模板采集器 - 玩家生命值数字采集")
root.geometry("600x400")

# 标题说明
title_label = tk.Label(root, text="=== 玩家生命值模板采集器 ===\n请点击下方按钮采集数字模板\n采集前请手动在屏幕上显示对应数字", font=("Arial", 14))
